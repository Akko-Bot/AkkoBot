...
namespace AkkoBot.Commands.Modules.Utilities.Services
{
    ...
    public class PollService : ICommandService
    {
        ...
        public DiscordMessageBuilder GetPollResult(CommandContext context, DiscordMessage message, PollEntity poll)
        {
            ...
            {
                PollType.Simple => GenerateSimplePollResult(new DiscordMessageBuilder(), message, GetPollReactions(message, AkkoEntities.ThumbsUpEmoji, AkkoEntities.ThumbsDownEmoji).ToArray()),
                PollType.Numeric => GenerateNumericPollResult(context, new DiscordMessageBuilder(), message, poll, GetPollReactions(message, AkkoEntities.NumericEmojis.Take(poll.Answers.Length))),
