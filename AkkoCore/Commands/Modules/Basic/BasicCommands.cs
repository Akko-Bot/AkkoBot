...
public sealed class BasicCommands : AkkoCommandModule, IDisposable
{
    ...
    private const string _versionString = "AkkoBot v0.3.2-beta";
    ...
    public async Task UptimeAsync(CommandContext context)
    {
        ...
        var embed = new SerializableDiscordEmbed()
            ...
            .AddField(
                ...
                context.FormatLocalized("{0}: {1}", "users", _shardedClient.ShardClients.Values.SelectMany(x => x.Guilds.Values).SelectMany(x => x.Members).Distinct().Count(x => !x.Value.IsBot)),
                inline: true
            );
