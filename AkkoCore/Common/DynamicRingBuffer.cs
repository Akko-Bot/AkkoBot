...
namespace AkkoCore.Common
{
    ...
    public sealed class DynamicRingBuffer<T> : IList<T>, IReadOnlyList<T>
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a <see cref="DynamicRingBuffer{T}"/> with the default size.
        /// </summary>
        ...
        /// <summary>
        /// Initializes a <see cref="DynamicRingBuffer{T}"/>.
        /// </summary>
        /// <param name="collection">The collection of elements to initialize the buffer with.</param>
        public DynamicRingBuffer(IEnumerable<T> collection)
        ...
        /// <exception cref="ArgumentOutOfRangeException">Occurs when <paramref name="index"/> is greater than the buffer's capacity or less than 0.</exception>
        public DynamicRingBuffer(int capacity, IEnumerable<T> collection)
        {
            if (capacity <= 0)
                throw new ArgumentOutOfRangeException(nameof(capacity), $"Capacity cannot be equal or lower than zero. Value: {capacity}");
        ...
        /// <summary>
        /// Removes all items from the <see cref="DynamicRingBuffer{T}"/> by setting them to
        ...
        /// <summary>
        ///  Determines whether the <see cref="DynamicRingBuffer{T}"/> contains a specific value.
        /// </summary>
        /// <param name="item">The object to locate in the <see cref="DynamicRingBuffer{T}"/>.</param>
        /// <returns><see langword="true"/> if item is found, <see langword="false"/> otherwise.</returns>
        public bool Contains(T item)
            => _internalList.Contains(item);

        /// <summary>
        /// Copies the elements of the <see cref="DynamicRingBuffer{T}"/> to an <see cref="Array"/>, starting at a particular index.
        /// </summary>
        /// <param name="array">The one-dimensional <see cref="Array"/> that is the destination of the elements copied from the <see cref="DynamicRingBuffer{T}"/>.</param>
        /// <param name="arrayIndex">The zero-based index in array at which copying begins.</param>
        /// <exception cref="ArgumentException">
        /// Occurs when the number of elements in the source <see cref="DynamicRingBuffer{T}"/> is greater than the available space from <paramref name="arrayIndex"/>
        ...
        /// <summary>
        /// Determines the index of a specific item in the <see cref="DynamicRingBuffer{T}"/>.
        /// </summary>
        /// <param name="item">The object to locate in the <see cref="DynamicRingBuffer{T}"/>.</param>
        ...
        /// <summary>
        /// Inserts an item to the <see cref="DynamicRingBuffer{T}"/> at the specified index.
        /// </summary>
        /// <param name="index">The zero-based index at which item should be inserted.</param>
        /// <param name="item">The object to insert into the <see cref="DynamicRingBuffer{T}"/>.</param>
        ...
        /// <summary>
        /// Adds an item to the <see cref="DynamicRingBuffer{T}"/>.
        /// </summary>
        ...
        /// <summary>
        /// Removes the first occurrence of a specific object from the <see cref="DynamicRingBuffer{T}"/>.
        /// </summary>
        /// <param name="item">The object to remove from the <see cref="DynamicRingBuffer{T}"/>.</param>
        /// <returns><see langword="true"/> if item was successfully removed from the <see cref="DynamicRingBuffer{T}"/>, <see langword="false"/> otherwise.</returns>
        ...
        /// <returns>The number of elements removed from the <see cref="DynamicRingBuffer{T}"/>.</returns>
