...
{
    public sealed class DynamicRingBufferTest
    {
        [Theory]
        [InlineData(null)]
        [InlineData(10)]
        ...
        private DynamicRingBuffer<T> CreateDynamicRingBuffer<T>(int? capacity, IEnumerable<T> collection = default)
        {
            return (capacity == default && collection == default)
                ? new DynamicRingBuffer<T>()
                : (capacity != default && collection != default)
                    ? new DynamicRingBuffer<T>(capacity.Value, collection)
                    : (capacity == default)
                        ? new DynamicRingBuffer<T>(collection)
                        : new DynamicRingBuffer<T>(capacity.Value);
        }
    }
}