...
build_akko()
{
    ...
    printf '\n> Building Akko for %s\n' "$2"
    dotnet publish --self-contained --nologo -c Release "$1" -r "$2" -p:PublishSingleFile=true
}
...
build_cog()
{
    ...
    printf '\n> Building cog "%s"\n' "$(basename "$1")"
    dotnet publish --nologo -c Release "$1"
}
...
cleanup_publish()
{
    ...
    local -r pattern="$1/!(AkkoBot.exe|AkkoBot|Data)" # This variable is needed because Bash whines otherwise
    rm -rf "$pattern"
...
create_pack()
{
    ...
    # set local -r variable to $3, else set to $4
    local -r pack_name=$( (( $# <= 3 )) || [[ -z $4 ]] && echo "$3" || echo "$4" )
...
# Variables
BOT_VER=$(get_input 'Type the version of the bot (eg. 0.3.5-beta): '); declare -r BOT_VER
declare -r AKKO_DIR='./AkkoBot'
declare -r PUBLISH_DIR="${AKKO_DIR}/bin/Release/net7.0"
...
[[ $(command -v tree) ]] || printf 'Removed %s.\n' "$(tree $PUBLISH_DIR -a | tail -n 1)"
rm -rf $PUBLISH_DIR
